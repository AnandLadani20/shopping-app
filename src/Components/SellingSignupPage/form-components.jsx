...
// export const signatureDrawer = fieldRenderProps => {
//   const {
//     validationMessage,
//     touched,
//     id,
...
//     <SignatureDraw valid={valid} id={id} disabled={disabled} ariaDescribedBy={`${hintId} ${errorId}`} {...others} />
//     {showHint && <Hint id={hintId}>{hint}</Hint>}
//     {showValidationMessage && <Error id={errorId}>{validationMessage}</Error>}
//   </FieldWrapper>
// }
...
export const FormsignatureUpload = fieldRenderProps => {
  ...
  return <FieldWrapper>
    ...
    <Upload id={id} autoUpload={false} showActionButtons={false} valid={valid} multiple={false} files={value} onAdd={onChangeHandler} onRemove={onRemoveHandler} ariaDescribedBy={`${hintId} ${errorId}`} ariaLabelledBy={labelId} {...others} />
    {showHint && <Hint id={hintId}>{hint}</Hint>}
    {showValidationMessage && <Error id={errorId}>{validationMessage}</Error>}
  </FieldWrapper>;
};

export const SignatureDraw = fieldRenderProps => {
  const {
    value,
    id,
    optional,
