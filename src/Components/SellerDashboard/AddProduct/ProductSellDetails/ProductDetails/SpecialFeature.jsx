...
const SpecialFeature = ({ specialFeatureOptions }) => {
  const { control, setValue, getValues } = useFormContext()
  const [fields, setFields] = useState([{ id: 'specialFeature', isFocused: false }]);

  const addField = () => {
    setFields([...fields, { id: `specialFeature${fields.length}`, isFocused: false }]);
  };

  const removeField = () => {
    if (fields.length > 1) {
      setFields(fields.slice(0, -1));
      getValues('specialFeature').pop();
    }
  };
  ...
  return (
    ...
    <div className="row" style={{ marginTop: "10px" }}>
      <div className="col-4 col-sm-3 col-lg-3 text-end">
        <label className="labelbold me-2">Special Feature</label>
      </div>
              ...
              render={({ field }) => (
                ...
                >
                  ...
                  {fieldData.isFocused ? <OptionsList setValue={setValue} fieldId={field.name} setIsFocused={(index, value) => {
                    ...
                    setFields(newFields);
                  }} index={index} options={specialFeatureOptions} /> : ""}
