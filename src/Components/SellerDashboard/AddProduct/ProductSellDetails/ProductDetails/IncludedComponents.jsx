...
const IncludedComponents = ({ includedComponentOptions }) => {
  const { control, setValue, getValues } = useFormContext()
  const [fields, setFields] = useState([{ id: 'includedComponent', isFocused: false }]);

  const addField = () => {
    setFields([...fields, { id: `includedComponent${fields.length}`, isFocused: false }]);
  };

  const removeField = () => {
    if (fields.length > 1) {
      setFields(fields.slice(0, -1));
      getValues('includedComponent').pop();
    }
  };
              ...
              render={({ field }) => (
                ...
                >
                  ...
                  {fieldData.isFocused ? <OptionsList setValue={setValue} fieldId={field.name} setIsFocused={(index, value) => {
                    ...
                    setFields(newFields);
                  }} index={index} options={includedComponentOptions} /> : ""}
