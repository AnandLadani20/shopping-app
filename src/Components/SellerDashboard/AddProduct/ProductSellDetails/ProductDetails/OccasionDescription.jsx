...
const OccasionDescription = ({ occasionDescriptionOptions }) => {
    const { control, setValue ,getValues} = useFormContext()
    const [fields, setFields] = useState([{ id: 'occasionDescription', isFocused: false }]);

    const addField = () => {
        setFields([...fields, { id: `occasionDescription${fields.length}`, isFocused: false }]);
    };

    const removeField = () => {
        if (fields.length > 1) {
            setFields(fields.slice(0, -1));
            getValues('occasionDescription').pop();
        }
    };
    ...
    return (
        ...
        <div className="row" style={{ marginTop: "10px" }}>
            <div className="col-4 col-sm-3 col-lg-3 text-end">
                <label className="labelbold me-2">Occasion description</label>
            </div>
            <div className="col-8 col-sm-7 col-lg-6">
                <div style={{ width: "100%" }}>
                    {fields.map((fieldData, index) => (
                        <Controller
                            key={fieldData.id}
                            name={`occasionDescription[${index}]`}
                            control={control}
                            defaultValue=""
                            render={({ field }) => (
                                <div
                                ...
                                >
                                    ...
                                    {fieldData.isFocused ? <OptionsList setValue={setValue} fieldId={field.name} setIsFocused={(index, value) => {
                                        ...
                                        setFields(newFields);
                                    }} index={index} options={occasionDescriptionOptions} /> : ""}
